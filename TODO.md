...
### 2. Round 4 — Nested Laplace strategic differentiator
**State:** ICAR / BYM2 / SPDE single-hyperparam paths (`cpp_nested_laplace_*`)
exist; the *generic* infrastructure does not.
**What:** items 13–15 of `plan.md`.
- **13.** Outer hyperparameter grid + numerical integration over θ, generic over
  the spatial backend.
- **14.** Inner Laplace at each grid point, warm-started from the previous
  point (CHOLMOD symbolic factor reuse).
- **15.** Posterior marginal extraction for both latent field and
  hyperparameters.
**Math reference:** `nested_laplace_proposal.md`. **Engineering:** `features_plan.md`.
...
1. **#2 Round 4 nested Laplace** — strategic differentiator (the headline
   piece). Math + engineering refs already in `nested_laplace_proposal.md`
   and `features_plan.md`; ICAR / BYM2 / SPDE single-hyperparam paths
   already exposed via `cpp_nested_laplace_*`.
...
4. **#7 CUDA live test** — schedulable anytime; needs a GPU build.

(Items #1 and #11 were retired by the 2026-04-29 audit — both turned out to
be implemented already; only the deprecated-path BYM2 H-mode verification
remains open and is low priority.)
