...
**`tgmrf()` user-defined GMRF latent block — shipped (P0–P3, P5–P9; only P4 CCD optimisation open).**

The user-facing surface and full inference family are in place. See
`generic-todo.md` for the design and phase-by-phase status. Files:

- Constructors: `R/tgmrf.R` (R closures), `R/tgmrf_cpp.R` (templated-C++
  via `Rcpp::sourceCpp` + registry).
- Inference adapters: `R/tgmrf_imh.R` (Tier-1 IMH-Laplace),
  `R/tgmrf_nuts.R` (Tier-1 outer-theta NUTS over `theta`),
  `R/tgmrf_nuts_joint.R` (Tier-1 joint NUTS over `(beta, z, theta)`,
  C++ backend only), `R/tgmrf_vi.R` (Tier-2 Pathfinder).
- Core C++: `src/tgmrf_nuts.cpp`, `src/tgmrf_registry.{h,cpp}`,
  `inst/include/tulpa/tgmrf.h`. Nested-Laplace branch in
  `src/nested_laplace_multi.cpp` reads the same precomputed-Q ABI for
  both R and C++ backends.
- Tests: `tests/testthat/test-tgmrf*.R` (7 files, ~150+ tests; recovery
  + R-vs-C++ equivalence + joint-vs-outer-theta NUTS agreement).
- Vignette: `vignettes/tgmrf.Rmd`.
- ABI: `TULPA_ABI_VERSION` bumped 20 → 21 for the
  `tulpa_register_tgmrf` callable.

Open follow-ons: P4 (CCD around the pilot mode), joint NUTS for
R-closure blocks (no clean math path; would require R callback per
leapfrog step), joint NUTS with random effects, joint NUTS with
multiple tgmrf blocks. None are blockers for the current user-facing
feature.
