...
#'   the engine's own default axis. Declines gracefully (keeps the fixed-grid
...
#'   presence: a spec marked with [auto_grid()], or one
...
#'     design. Engages only on the tensor path (the
...
#'     the contraction of the cell third-derivative tensor, which differences
#'     the cross-arm Hessian the coupling spec already returns. A fit that can
#'     carry neither reports `NaN`, not a silently wrong 0, and
#'     `$inner_skew_declined` says WHY -- `"coupled_arm"` marks arms the inner
#'     layer could not score at all, distinct from a diagnostic that was simply
#'     switched off -- while `$inner_skew_arms_declined` names the arms with no
#'     oracle, including on a partially scored fit. See
...
#'     relocation itself (`centre_unreliable`) is off (`Inf`):
#'     every finite cutoff was measured to decline the coefficients the
#'     correction helps most.
...
#'     the bit -- a declined coefficient keeps the grid-mixture read. See
#'     [tulpa_nested_laplace()] for the measurement behind the default.
...
#'     rest at their Gaussian conditional. Settings and semantics are the ones
...
#'     inner Pareto-k-hat; where that also bands the
...
#'     `weights` / `log_marginal` with `weights_source` reporting `"cila"`; the
#'     pre-correction pair is kept as `$cila$laplace` and `$cila$retained_mass`
#'     is the original share of the cells that produced a usable particle set.
#'     `$cila` also carries the variant actually run and the PSIS grade
#'     (`pareto_k`, `rel_ess`) of the correction's own weights. A cell whose
#'     inner solve factorized SPARSELY draws through the CHOLMOD factor's own
#'     triangular and permutation solves; an LDL' factor carries no square root
#'     to draw with and is declined with `"sparse_factor_not_ll"`.
...
#'   * `pareto_k_declined` -- when `pareto_k` is `NA`, WHY:
...
#'      the axis was actually laid from. A clamped axis was
...
#'      posterior. Every nested path stamps the pair, not only
...
#'      `"mirrored_edge_not_representable"` when it is not.
...
.tulpa_nl_joint_once <- function(responses, prior, copy = NULL, phi_grid = NULL,
    ...
    # Inner-Laplace skewness diagnostic, the complementary
    ...
    # intervals. `integration` names how the grid was BUILT;
    ...
    # Per-cell fixed-effect covariance retention. The block
    ...
    # rather than off a grid-wide store of every precision.
...
# Inner-Laplace skewness diagnostic for the joint driver:
...
# counterpart, `.nlj_multi_inner_skew_at_theta()`.
