...
#' @template phi
#' @param offset Optional per-observation offset added to the linear predictor
#'   (length `length(y)`), as an `offset()` term in a model formula. Every
#'   single- and multi-block kernel carries it, and `fitted_eta` includes it.
...
#'     The band that bounded the relocation itself (`centre_unreliable`) is
#'     off (`Inf`): scored over seven fixtures with an exact
...
#'
#'     IT IS ON BY DEFAULT, so `summary()` / `confint()` on a
...
#'     at their Gaussian conditional. `TRUE` takes every default; a list
#'     overrides `band`
...
tulpa_nested_laplace <- function(y, n_trials, X, prior = NULL,
  ...
  # Corrected integrated Laplace, the second inner debias.
  ...
  # intervals are read off. A numerical reconstruction of the
  ...
  prior <- .prov$prior
  # And refuse an axis this path cannot read: a grid field
  ...
  if (!is.null(offset)) {
    ...
    if (length(offset) != length(y) || any(!is.finite(offset))) {
      stop("`offset` must be a finite numeric vector of length length(y) (",
           length(y), ").", call. = FALSE)
    }
  }
  ...
  if (.is_multi_block_prior(prior)) {
    ...
    # own fields and its neighbours keep theirs; a prior whose blocks the
    # family table does not cover declines with a reason rather than leaving
    # the fit with no placement record at all.
  ...
  # a single mode-Hessian recenter-and-refit when the fixed default axis
...
.nl_attach_pareto_k <- function(res, prior, cargs, dispatch_kind, type,
  ...
  # The outer-integration regime is read off the stored grid
  ...
  # Every decline below says which one it was.
...
# zero-length vector and takes the session down. Checking
...
.nl_attach_grid_hessians <- function(res, p_fixed, keep_joint = FALSE,
  ...
  # gcol33/tulpa#901) and into an intrinsic temporal field's marginal variance.
  # The driver names each intrinsic block's constraint group
  # (`constraint_cols`), and both reads are conditioned on them by the kriging
  # correction a joint fit's retained block and posterior draws use
  # (cpp_joint_inner_vcov_blocks, Rue & Held 2005):
  #   Cov(x | A x = 0) = V - W (A W)^-1 W',  V = Q^-1 E,  W = Q^-1 A'.
  # The mode needs no correction: the centring already put it on the
  # constraint.
  constr <- res$constraint_cols
  n_con <- length(constr)
  if (n_con) {
    E <- cbind(E, vapply(constr, function(cc) {
      a <- numeric(n_x)
      a[as.integer(cc)] <- 1
      a
    }, numeric(n_x)))
  }
  sel <- seq_len(p_fixed + n_var)
  ...
  for (k in seq_len(n_grid)) {
    ...
    # V = Qk^{-1} E selects the fixed-effect (and read field) columns of the
    # joint inverse. A dense GP / NNGP field precision at close locations makes
    # the joint precision ill-conditioned, which trips Matrix's
    # condition-number guard. Retry with a negligible diagonal jitter (scaled
    # to the precision) so the marginal-SE extraction stays available rather
    ...
    rows <- c(seq_len(p_fixed), var_cols)
    S <- V[rows, sel, drop = FALSE]
    if (n_con) {
      W  <- V[, p_fixed + n_var + seq_len(n_con), drop = FALSE]
      AW <- crossprod(E[, p_fixed + n_var + seq_len(n_con), drop = FALSE], W)
      Wr <- W[rows, , drop = FALSE]
      S  <- S - Wr %*% solve((AW + t(AW)) / 2, t(Wr))
    }
