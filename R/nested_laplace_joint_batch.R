...
# Fit B species through one fused grid solve.
#
# `fits` is a list of functions of no arguments, each performing one ordinary
# joint nested-Laplace fit whose main grid solve differs from the others' only
# in the per-species parts of the request (arm responses, arm dispersions,
# dispersion-axis nodes). Returns the list of fit results in order. The fits
# replay one after another from the random number state the batch was called
# at, so they draw the stream the same fits called in sequence draw; each
# capture starts from that state too, and a fit whose grid request depended on
# a draw taken before it is refused at its replay rather than served. Warnings
# and messages a capture run raises are muffled: the replay raises them again.
#
# Raises a `tulpa_grid_batch_ineligible` error when the fits do not share a
# design or a request uses a setting the fused driver does not carry. The random
# number state is put back to where the batch was called before that error
# leaves, so a caller that falls back to fitting the species one at a time draws
# the stream it would have drawn without the batch.
# @keywords internal
