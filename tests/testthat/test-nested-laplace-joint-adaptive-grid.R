# Recovery test for the adaptive sigma_pos-grid refinement path added to
...
#
# The test rig deliberately gives the user a `sigma_pos_grid` whose maximum
# (0.6) sits below the true sigma_pos generating the data (~2.0 with
# alpha_true = 2.0 and sigma = 1.0). With `adaptive_grid = FALSE` the
# joint posterior is truncated at the boundary and the derived alpha mean
# collapses near the largest achievable ratio sigma_pos_max / sigma_occ_min.
# With `adaptive_grid = TRUE` the refinement helper detects boundary mass
# on the sigma_pos axis, appends densification + outward extension points,
# and re-evaluates the kernel on those new triples. The resulting posterior
# shifts outward and the `adaptive_grid_info` field documents the action.
...
.fit_joint_icar <- function(sim, sigma_pos_grid, adaptive_grid) {
...
test_that("adaptive_grid = FALSE leaves grid and result unchanged", {
    ...
    fit <- .fit_joint_icar(sim, sigma_pos_grid = c(0.2, 0.4, 0.6),
                            adaptive_grid = FALSE)
    expect_null(fit$adaptive_grid_info)
    expect_equal(length(fit$log_marginal), 9L)            # 3 sigma_pos x 3 sigma_occ
    expect_equal(max(fit$theta_grid[, "sigma_pos"]), 0.6) # boundary intact
})


# --------------------------------------------------------------------------- #
# 2. adaptive_grid = TRUE refines beyond the user's sigma_pos boundary         #
#    when the truncated grid traps posterior mass at the edge                  #
# --------------------------------------------------------------------------- #

test_that("adaptive_grid = TRUE extends sigma_pos when boundary carries mass", {
    sim <- .simulate_joint_icar_strong(seed = 6)
    fit_F <- .fit_joint_icar(sim, sigma_pos_grid = c(0.2, 0.4, 0.6),
                              adaptive_grid = FALSE)
    fit_T <- .fit_joint_icar(sim, sigma_pos_grid = c(0.2, 0.4, 0.6),
                              adaptive_grid = TRUE)

    # Refinement metadata is populated.
    expect_false(is.null(fit_T$adaptive_grid_info))
    expect_true("sigma_pos" %in%
    ...
    expect_lt(fit_F$theta_mean[["alpha"]], 1.0 + 1e-6)
...
test_that("adaptive_grid = TRUE leaves grid alone when boundary is empty", {
    sim <- .simulate_joint_icar_strong(seed = 6)
    # Wide grid: well past the true sigma_pos ~ 2.0 (boundary near 4 has
    # negligible weight under any seed of this simulator).
    sp_wide <- c(0.1, 0.3, 0.6, 1.0, 1.5, 2.5, 4.0)
    fit_F <- .fit_joint_icar(sim, sigma_pos_grid = sp_wide,
                              adaptive_grid = FALSE)
    fit_T <- .fit_joint_icar(sim, sigma_pos_grid = sp_wide,
                              adaptive_grid = TRUE)
