...
and `conditioned_on` (which log-scale quantity
[`logLik()`](https://rdrr.io/r/stats/logLik.html) reports, and the
hyperparameters it holds fixed; fits that disagree on either are refused
rather than ranked). For `"waic"` / `"loo"` (ranked best-first):
`model`, `elpd`, `se_elpd`, `p_eff`, `ic` (`-2 * elpd`), `delta` (elpd
gap to the best model), `se_diff` (SE of that pointwise elpd
difference), and `weight` (the Akaike-style weight on the criterion).
