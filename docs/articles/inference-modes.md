...
the spatial fit earlier reported a Gibbs backend. It returns posterior
draws, so the fixed effects come through
[`coef()`](https://rdrr.io/r/stats/coef.html) and the field and its
precision are columns of the same draws matrix, named in `param_names`.

``` r

round(coef(fit_auto), 3)
tail(fit_auto$param_names, 3)
round(fit_auto$means[c("log_tau_spatial", "phi_spatial[1]")], 3)
```
...
Laplace and Pathfinder do not report the same quantity under
[`logLik()`](https://rdrr.io/r/stats/logLik.html): the Laplace tier
gives a log marginal likelihood and Pathfinder a mean log posterior.
Their values are therefore not a model comparison, and
[`compare_models()`](https://gillescolling.com/tulpa/reference/compare_models.md)
refuses to rank quantities on different scales.
