...
The pointwise structure is the part the engine cannot supply on its own,
and the reason is worth stating. WAIC estimates the out-of-sample
...
## modelAverage() for prediction

When no single model clearly wins, averaging their predictions can beat
committing to one. `modelAverage()` takes named fits, computes a weight
...
```{r model-average}
ma <- modelAverage(slope = m1, slope_re = m2, full = m3,
                   criterion = "waic")
ma$weights
head(round(ma$averaged, 3))
```

The weighting deserves a plain statement. The weights here come out equal
because the WAIC weight needs a pointwise log-likelihood, which a base
engine fit does not carry, so `modelAverage()` falls back to a uniform
split across the models. A uniform average is still a useful hedge when
the candidates disagree, but it is not an evidence-weighted one. To
weight by predictive accuracy, route the fits through a model package
that populates the pointwise log-likelihood, and `modelAverage()` then
draws genuine WAIC weights from the same `compare_models()` it calls
internally. The `averaged` vector is the per-observation blend you would
use as a combined prediction; `ma$comparison` is the table the weights
were read from.
