...
samplers. The nested-Laplace integrator states its hyperpriors on its own outer
grid, and every default axis there carries a proper prior. A field's standard
deviation, variance or precision (`icar`, `rw1`, `rw2`, `ar1`'s `tau`, `bym2`
and `iid`'s `sigma`, a GP variance) takes the PC prior `P(sigma > 3) = 0.01`,
carried to whichever of the three the axis integrates. That anchor is wider than
the sampler slot's `P(sigma > 1) = 0.01`: on a binary response with a field of
standard deviation 3, the tighter anchor pulled the posterior mean down to about
...
coordinates' bounding box. A correlation or mixing weight takes a uniform on its
domain, and a negative-binomial size takes the PC prior on its overdispersion
that R-INLA ships as `pc.mgamma`. [tulpa_re_cov_nested()] and [tulpa_eb()] use
the same scale anchor through the PC + LKJ prior (`hyperprior = "pc_lkj"`, the
default; see [re_cov_pc_lkj_prior()]); `hyperprior = "flat"` removes it, which
for [tulpa_eb()] gives the unpenalized maximum marginal likelihood.

Because these priors are proper, `logLik()` on a nested fit is the evidence under
them and does not depend on where the grid was laid. An axis the engine has no
proper prior for, such as a beta precision or a gamma shape, leaves the grid's
own measure in place, and `logLik()` then reports `NA` naming that axis.
[tulpa_re_cov_gibbs()] needs a proper prior for its conjugate Inverse-Wishart
`Sigma | b` step and defaults to the weakest one (`prior_df = n_coefs + 1`).
