...
// ----------------------------------------------------------------------------
// Stub fallbacks used until the sibling agent's
// compute_log_prior / compute_log_lik_only land. Once those exist in
// hmc_sampler.h they can be called directly; the stubs use the
// `skip_obs_loop` flag of compute_log_post.
//
// log_prior(theta) = log_post(theta, skip_obs_loop=true)
// log_lik(theta)   = log_post(theta) - log_post(theta, skip_obs_loop=true)
// ----------------------------------------------------------------------------
inline double smc_log_prior_stub(
    const std::vector<double>& params,
    const ModelData& data,
    const ParamLayout& layout
) {
    return tulpa_hmc::compute_log_post(params, data, layout, /*skip_obs_loop=*/true);
}

inline double smc_log_lik_only_stub(
    const std::vector<double>& params,
    const ModelData& data,
    const ParamLayout& layout
) {
    double full   = tulpa_hmc::compute_log_post(params, data, layout, /*skip_obs_loop=*/false);
    double prior  = tulpa_hmc::compute_log_post(params, data, layout, /*skip_obs_loop=*/true);
    return full - prior;
}
...
inline SMCDriverResult run_smc_sampler(
    ...
    // Closures: prior + likelihood (use stubs for now; sibling agent's
    // factoring will replace these with direct calls).
    auto log_prior_fn = [&data, &layout](const std::vector<double>& theta) -> double {
        return smc_log_prior_stub(theta, data, layout);
    };
    auto log_lik_fn = [&data, &layout](const std::vector<double>& theta) -> double {
        return smc_log_lik_only_stub(theta, data, layout);
    };
