...
bool SparseCholeskySolver::factorize_with_ridge_retry(
    cholmod_sparse* A, double ridge_init, int max_retries
) {
    if (factorize(A)) return true;
    if (!A) return false;

    // Supernodal failed: matrix is (numerically) rank-deficient. Fall back
    // to simplicial LDL' with dbound = ridge_init so |D_jj| < ridge_init is
    // clamped to ridge_init. This mirrors the dense path's clamp in
    // cholesky_factorize_impl_raw (`if (sum <= 0) sum = 1e-6`).
    //
    // Use natural (identity) ordering so the column-elimination order
    // matches the dense path. CHOLMOD's default AMD permutation would
    // shuffle the rank-deficient direction's pivot to a different column
    // and produce a permutation-dependent log_det that diverges from the
    // dense baseline by O(1) units.
    int saved_super = common_.supernodal;
    int saved_nmethods = common_.nmethods;
    int saved_ordering = common_.method[0].ordering;
    common_.supernodal = CHOLMOD_SIMPLICIAL;
    common_.dbound = ridge_init;
    common_.final_ll = 0;  // keep LDL' (dbound only honored for LDL')
    common_.nmethods = 1;
    ...
    common_.supernodal = saved_super;
    common_.nmethods = saved_nmethods;
    common_.method[0].ordering = saved_ordering;

    // Suppress further numeric retries; once we have a simplicial factor,
    // subsequent factorize() calls reuse it with dbound, which is enough.
    if (!ok) {
        // True singularity even with dbound — last-resort uniform ridge.
        int n = static_cast<int>(A->nrow);
        int* Ap = static_cast<int*>(A->p);
        int* Ai = static_cast<int*>(A->i);
        double* Ax = static_cast<double*>(A->x);
        double cumulative = 0.0;
        double ridge = ridge_init;
        for (int retry = 0; retry < max_retries; retry++) {
            double delta_ridge = ridge - cumulative;
            for (int j = 0; j < n; j++) {
                int idx = Ap[j];
                if (idx < Ap[j + 1] && Ai[idx] == j) {
                    Ax[idx] += delta_ridge;
                }
            }
            cumulative = ridge;
